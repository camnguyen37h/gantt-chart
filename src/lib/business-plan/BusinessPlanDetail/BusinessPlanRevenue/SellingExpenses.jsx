...
const SellingExpenses = ({
  ...
  const handleAddedRowChange = (key, field, month, value) => {
    updateIsSaveConfirmShowed(true)
    dispatch(setCreateSellingExpensesData({ key, field, month, value }))
  }
    ...
    setDataSourceTable(prevData => {
      ...
      if (parent) {
        const newChildKey = `${record.key}-new-${
          SELLING_EXPENSES_TYPE_ID[record.key]
        }-${newChildIndex}`
  ...
  const handleRemoveChildRow = record => {
    ...
    if (record.key.includes('new')) {
      dispatch(setDeleteSellingExpensesData(record.key))
      ...
      if (removedItem) {
        ...
        dispatch(setDeleteSellingExpensesData(removedDataId))
      }
    }
  ...
  const columns = useCallback(
    (expandedKeys, toggleExpand) => {
      if (!mainData) return
      const totalExpenseValue = mainData.revenues.reduce((sum, item) => {
      ...
      return [
        ...
        },
        {
          ...
          children: [
            {
              title: 'Selling expenses categories',
        ...
        {
          title: (
            <span title={formatFloatNumber(totalExpenseValue, 0, 8)}>
              {formatFloatNumber(totalExpenseValue, 0, 3)}
            </span>
          ),
          children: [
            {
